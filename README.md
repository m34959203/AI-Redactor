...
AI-powered редактор для научных журналов с автоматической обработкой статей, проверкой орфографии и генерацией рецензий с использованием Claude AI.
...
- **Claude AI (Anthropic)** - обработка текста и AI-функции
- **Lucide React** - иконки
...
Откройте `.env` и добавьте ваш API ключ Anthropic:
...
3. Добавьте переменную окружения в Vercel dashboard:
   - `VITE_ANTHROPIC_API_KEY` = ваш API ключ
...
- 🚫 Не делитесь ключами API публично
...
- [Anthropic](https://www.anthropic.com/) за предоставление Claude API
